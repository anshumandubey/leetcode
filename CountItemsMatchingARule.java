...
public class CountItemsMatchingARule {
    static class Solution {
        public int countMatches(List<List<String>> items, String ruleKey, String ruleValue) {
            return (int) items.stream().filter(item -> ruleKey.equals("type") ? item.get(0).equals(ruleValue) : ruleKey.equals("color") ? item.get(1).equals(ruleValue) : item.get(2).equals(ruleValue)).count();
        }
    }
